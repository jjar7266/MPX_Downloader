...
def detect_nodejs():
    """
    Teacher: yt-dlp can use external JavaScript runtimes (like Node.js) to
    decrypt certain YouTube signatures. Without Node.js, some videos will fail
    with 403 errors. This helper checks whether Node.js is installed and
    available on PATH.

    Student: We run `node -v` quietly. If it returns a version, Node.js exists.
    """
    try:
        result = subprocess.run(
            ["node", "-v"],              # Ask Node.js for its version
            capture_output=True,         # Capture output instead of printing
            text=True,
            creationflags=CREATE_NO_WINDOW
        )
        return result.returncode == 0    # True if Node.js responded normally
    except Exception:
        return False                     # Node.js not installed or not on PATH
...
def download_mp3(url, save_path, status_label, root):
    ...
    # --------------------------
    # Build yt-dlp command (MP3)
    # --------------------------
    """
    Teacher: We construct the yt-dlp command dynamically. If Node.js is
    available, we add '--js-runtime node' to ensure signature decryption works
    for videos that require JavaScript evaluation.

    Student: We append the URL last to keep the command clean and readable.
    """
    ...
    # Add Node.js runtime support if available
    if root.has_nodejs:
...
def download_mp4(url, save_path, status_label, root):
    ...
    # --------------------------
    # Build yt-dlp command (MP4)
    # --------------------------
    """
    Teacher: Same logic as MP3. We build the command step-by-step and inject
    Node.js support only if detected. This ensures compatibility with YouTube's
    signature decryption system.

    Student: This format selection chooses the best MP4 video + best M4A audio.
    """
    command = [
        LOCAL_YTDLP,
        "-c",
...
def main():
    ...
    # ---------------------------
    # Node.js Detection (Startup)
    # ---------------------------
    """
    Teacher: yt-dlp can use external JavaScript runtimes (like Node.js) to
    decrypt certain YouTube signatures. Without Node.js, some videos will fail
    with 403 errors. We detect Node.js once at startup and store the result on
    the root object so download functions can read it.

    Student: We run 'node -v' quietly. If it returns a version, Node.js exists.
    """
    root.has_nodejs = detect_nodejs()

    if not root.has_nodejs:
        thread_safe_status(root, status_label, "Node.js not detected", delay=5000)
        ...
        logging.info("Node.js detected successfully.")
